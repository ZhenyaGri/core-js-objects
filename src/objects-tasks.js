...
function mergeObjects(objects) {
  return objects.reduce((previousValue, currentValue) => {
    ...
    Object.entries(currentValue).forEach(([key, value]) => {
      newValue[key] = (newValue[key] || 0) + value;
    });
    return newValue;
  }, {});
}
...
function makeWord(lettersObject) {
  const len = Object.values(lettersObject).flat();
  Object.keys(lettersObject).forEach((elem) => {
    lettersObject[elem].forEach((value) => {
      len[value] = elem;
    });
  });
  return len.join('');
...
function sellTickets(queue) {
  let money25 = 0;
  let money50 = 0;
  return queue.every((moneyCustomer) => {
    if (moneyCustomer === 25) {
      money25 += 1;
    } else if (moneyCustomer === 50) {
      if (money25 === 0) {
        return false;
      }
      money25 -= 1;
      money50 += 1;
    } else if (moneyCustomer === 100) {
